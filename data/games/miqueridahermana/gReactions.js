		...
		reaction: function (par_c) {
			...
			if (par_c.loc == primitives.IT_X("estudio")) {
				...
				if (primitives.IT_GetAttPropValue (par_c.loc, "generalState", "state") == "0") {
					...
					primitives.GD_CreateMsg (1, "sueño1", "No puedes respirar, estás encerrado en la cueva de la playa y no puedes escapar. De repente, un corrimiento de arena de la playa entra en la cueva y te cubre hasta la cintura y no puedes moverte. En tus manos tienes el collar de conchas marinas que has hecho para regalárselas a tu querido hermano...<br/>")
					primitives.CA_ShowMsg ("sueño1");

					primitives.GD_CreateMsg (1, "pulsa_avanzar", "Continuar") // se reutilizará en más sitios
   				    primitives.CA_PressKey ("pulsa_avanzar");
					
					primitives.GD_CreateMsg (1, "después_sueño1", "Pero no, no puede ser, estos no son recuerdos tuyos, sólo pueden ser los últimos recuerdos de ella, del día en que...<br/>Despiertas, mojado en sudor.<br/><br/>Te levantas, no crees que vayas a poder reconciliar el sueño. De hecho, temes volver a dormir y volver a vivir esa horrible experiencia.<br/><br/>");
		...
		reaction: function (par_c) {
			...
			if (par_c.target == primitives.IT_X("salón_comedor")) {
				...
				if (primitives.IT_GetAttPropValue (par_c.target, "generalState", "state") == "0") {
					  ...
					  primitives.GD_CreateMsg (1, "pulsa_para_entrar", "Pulsa para entrar");
...
usr.reseteoDia = function() {
	...
	primitives.GD_CreateMsg (1, "sueño2", "Estás durmiendo en tu piso de Nueva York. Te ves desde fuera, flotando encima de ti. Luego te fijas en la foto de la estantería donde estáis tú y tu hermana jugando en la playa. Al lado de la foto está el extraño libro que conseguiste en el anticuario de Marrakech. El libro se abre solo y las páginas comienzan a correr hasta detenerse en una extraña página, donde queda abierto. Finalmente, sales flotando de la habitación, atravesando la ventana.<br/>")
	primitives.GD_CreateMsg (1, "sueño3", "Entre brumas, ves caras de personas que sufren. Una de ellas te mira y se gira para que la sigas. Atraviesan juntos sitios de extraños de colores y sonidos mareantes y aparecen en el mercado de Marrakech. En trastienda la de un puesto, la figura te señala el libro.<br/>")
	primitives.GD_CreateMsg (1, "sueño4", "En el puesto del mercado de Marrakech. Te ves desde arriba. Como en una película, la escena muestra cómo observas muchos libros pero no coges ninguno. De repente, la cámara hace un travelling vertigionso al interior del dependiente, cuyos ojos pasan a ser la cámara de la película, que se retira a la parte trasera del puesto, la mano del mercader coge el libro y te lo ofrece.<br/>")
	primitives.GD_CreateMsg (1, "sueño5", "En el promontorio. Ves cómo tu madre llora desconsoladamente al lado de la tumba de tu querida hermana. De repente, ves cómo fija su mirada en ti, que te retiras hacia atrás, flotando sobre la rompiente mar. Tu madre se acerca a ti y al intentar abrazarte, cae irremediablemente al abismo. Vuelas tras ella, intentando salvarla, pero no puedes hacer nada.<br/>")
	primitives.GD_CreateMsg (1, "sueño6", "El día en que decidiste viajar a Marruecos. Sentado en tu mesa del salón del piso de Nueva York. Ves como si te hubieran grabado con una cámara web espía, cómo te levantas un momento del ordenador donde mirabas ofertas de viaje. La cámara web enfoca el ordenador y ves cómo aparece una oferta súper barata a Marrakech y Casablanca y cómo, en una segunda página web se abre un artículo sobre Lovecraft y el Necronomicón. La cámara web se retira y te apunta. Observas cómo pones cara de bobo al ver la oferta y el artículo y cómo metes la numeración de tu tarjeta de crédito.<br/>")
	...
	primitives.GD_CreateMsg (1, "después_sueño2a5", "Pero no, definitivamente estos no son recuerdos tuyos. Sientes a tu querida hermana muy cerca de ti, ¿te estarás volviendo loco?<br/><br/>");
	primitives.CA_ShowMsg ("después_sueño2a5");

	primitives.GD_CreateMsg (1, "día_s1", "<br/>Comienzas la jornada %s1 desde que tuviste la primera pesadilla.<br/><br/>");
...
usr.hablarConPNJ = function(PNJIndex, option, esTelefonico) {
	...
	if (typeof option == 'undefined') { // phase 1: asking dialog
		...
		if ( (+primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_pilas") == 0) &&
			...
			primitives.GD_CreateMsg (1, "pedir_cargador", "Pedir prestado un cargador");
	...
	if (option == "pedir_cargador") {
		primitives.GD_CreateMsg (1, "DLG_pedir_cargador_reacción", "Venga, va, pero no te lo olvides por ahí, que me da que eres un poco despistado.");
